  ...
  while ( off + 4 <= dmi.size() )
  {
    ...
    if ( type == 17 )
    {
      ...
      if ( mod.sizeMiB > 0 )
      {
        ...
        const uint8_t manufacturerIndex = ( len > 0x17 ) ? dmi[off + 0x17] : 0;
        const uint8_t serialIndex = ( len > 0x18 ) ? dmi[off + 0x18] : 0;
        const uint8_t partIndex = ( len > 0x1A ) ? dmi[off + 0x1A] : 0;
        ...
        mod.manufacturer = readSmbiosString( dmi.data() + strStart, end - strStart, manufacturerIndex );
        mod.serialNumber = readSmbiosString( dmi.data() + strStart, end - strStart, serialIndex );
        mod.partNumber = readSmbiosString( dmi.data() + strStart, end - strStart, partIndex );
